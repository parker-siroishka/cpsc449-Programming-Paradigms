...
getFAccepted (x:xs) (w:ws) (y:ys) accepted
    | (stringIsAccepted (x:xs) y) && stringIsNotForbidden (w:ws) y = getFAccepted (x:xs) (w:ws) ys (accepted ++ [y])
    | otherwise = getFAccepted (x:xs) (w:ws) ys accepted
